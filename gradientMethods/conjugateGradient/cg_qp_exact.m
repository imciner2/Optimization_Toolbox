function [ x_final, res, k, x_it, p_it, r_it ] = cg_qp_exact(A, b, x0, varargin)
%CG_QP_EXACT Minimize the given quadratic function using the conjugate
%gradient method in exact arithmetic
%
...
% The computation simulates the CG with exact arithmetic by explicitly
% orthogonalizing the direction vector against all previous direction
% vectors using the Gram-Schmidt orthogonalization process at every step.
%
%
% Usage:
%   [ x_final, res, k ] = CG_QP_EXACT( A, b, x0 );
%   [ x_final, res, k ] = CG_QP_EXACT( A, b, x0, ... );
%   [ x_final, res, k, x_it, p_it, r_it ] = CG_QP_EXACT( A, b, x0, 'SaveIterates', 1, ... );
%
% Inputs:
...
if ( mod(numArgs, 2) ~= 0 )
    error('cg_qp_exact: Incorrect number of arguments');
end
...
while ~STOP
    ...
    % Orthogonalize the new residual vector if previous ones exist
    if (k > 1)
        r_n = gram_schmidt(r_n, r_pre(:,1:k-1));
    end
    ...
    if ( any( isnan(x_n) ) )
        error('cg_qp_exact: NaN encountered in the variables, stopping');
    end
    ...
    if ( (MAX_ITER ~= 0) && (k == (MAX_ITER+2) ) )
        warning('cg_qp_exact: Maximum interations reached before tolerance');
        STOP = 1;
    end
